...
export default function PortraitGenerator({
  ...
  const [active, setActive] = useState([{
    ...
    artifacts: [{
      ...
      path: `/portrait-chibi/img/artifacts/icon/Crimson_Witch_of_Flames.png`
    }],
    weapon: {
      name: "Staff_of_Homa",
  ...
  function add(icon: PortraitIcon, multi: boolean, note: boolean) {
    ...
    // Identificar si es artefacto o arma
    const isArtifact = icon.path.includes('/artifacts/icon/')
    const isWeapon = icon.path.includes('/weapons/icon_ascended/')
    if (multi && (isArtifact || isWeapon)) {
      ...
      if (isArtifact) {
        // Agregar artefacto (pueden ser varios)
        if (!last.artifacts) last.artifacts = []
        last.artifacts.push(icon)
